# Rashtrafarm — Play Store Launch Guide & Progress

Everything needed to take **Rashtrafarm** from code to Google Play, plus
...
- **Status (Aug 2026):** renamed from "Fatima Goat Farm" to **Rashtrafarm**
  ...
  site, run one build, start closed testing.
...
2. Set **Project Name** to exactly `rashtrafarm` — that is what makes the
...
4. **Deploy.** Then open both URLs in a browser and confirm they load:
   - `https://rashtrafarm.vercel.app/privacy-policy.html`
   - `https://rashtrafarm.vercel.app/delete-account.html`
...
```
CORS_ORIGIN=<whatever is already there>,https://rashtrafarm.vercel.app
```
...
The rename also lands in this build:

- `name` is `Rashtrafarm`, so that is what appears under the launcher icon.
- `package` is `com.rashtrafarm.app`.
- The deep-link scheme is `rashtrafarm://`, and the backend now writes
  `rashtrafarm://goat/<token>` into newly generated QR tags. **Tags already
  printed under the old `goatfarm://` scheme still scan** — `parseToken` in
  `ScanGoatScreen.tsx` matches on `goat/<token>` and ignores the scheme, which
...
**This matters more than it did under the old name.** "Rashtrafarm" is a coined
word — nobody searches for it, and nothing in it tells Play what the app is for.
Under the previous name the words "Goat" and "Farm" were carried by the brand
itself; now they have to come from the suffix, or the listing is unreachable by
anyone who does not already know the name. Take the suffix.

The name under the launcher icon stays `Rashtrafarm` either way — that comes
from `app.json`, not this field.
...
An account is required to use the app. If your farm already uses Rashtrafarm, ask your owner or manager for a login.
```
...
4. Once Google approves the build: **Manage track → Testers → Copy link**. It
   looks like `https://play.google.com/apps/testing/com.rashtrafarm.app`.
